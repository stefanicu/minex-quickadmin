                    ...
                    @foreach ($blogs10 as $bg)
                            <li class="list-group-item flex-row {{ $blog->id === $bg->id ? 'active' : '' }}">
                                <div class="d-flex w-100 justify-content-between">
                                    ...
                                    @endif
                                    <a href="{{ route('blog.index', ['slug' => $bg->slug, 'page' => request()->get('page')]) }}" class="flex m-auto">
                                        {{ $bg->name }}
                                    </a>
                                </div>
                            </li>
        ...
        <div class="bxslider-related mx-auto pt-4">
            @foreach ($blogs as $blg)
                <div class="d-flex slider-image-container">
                    <a href="{{ route('blog.index', ['slug' => $blg->slug, 'page' => request()->get('page')]) }}" class="blogim position-relative">
...
@section('scripts')
    @parent
    <script>
        $(document).ready(function(){$(function(){$('.bxslider').bxSlider({mode:'fade',slideWidth:345});});
            $(function(){$('.bxslider-related').bxSlider({minSlides:1,maxSlides:2,slideWidth:345,infiniteLoop:true,slideMargin:5,pager:false});});});
        $(function(){$('.bxslider-img').bxSlider({mode:'fade',slideWidth:345,pager:true,controls:true});});
    </script>
@endsection
