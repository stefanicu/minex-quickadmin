<section class="container-fluid mx-0 px-0 section section__left pt-4 section3">
    <figure class="d-flex align-items-center section--figure">
        <picture data-alt="s3">
            ...
            <img class="lozad img-fluid section--hero-img lazy-fade" srcset="{{ asset('/img/home/s3/xl-min.jpg') }}"
            ...
            @if($consultancy)
                ...
                <ul>
                    @foreach($consultancy_references as $consultancy_reference)
                        <li>
                            <a href="{{ route('reference.'.app()->getLocale(), ['slug' => $consultancy_reference->slug]) }}">{{ $consultancy_reference->name }}</a>
                        </li>
                    @endforeach
                </ul>
            @endif
