    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.homes.update", [$home->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.home.fields.language') }}</label>
                <select class="form-control {{ $errors->has('language') ? 'is-invalid' : '' }}" name="language" id="language" required>
                    ...
                    @foreach(App\Models\Home::LANGUAGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('language', $home->language) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.home.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $home->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.home.fields.first_text_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="seccond_text">{{ trans('cruds.home.fields.seccond_text') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('seccond_text') ? 'is-invalid' : '' }}" name="seccond_text" id="seccond_text">{!! old('seccond_text', $home->seccond_text) !!}</textarea>
                @if($errors->has('seccond_text'))
                    <span class="text-danger">{{ $errors->first('seccond_text') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.home.fields.seccond_text_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="quote">{{ trans('cruds.home.fields.quote') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('quote') ? 'is-invalid' : '' }}" name="quote" id="quote">{!! old('quote', $home->quote) !!}</textarea>
                @if($errors->has('quote'))
                    <span class="text-danger">{{ $errors->first('quote') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.home.fields.quote_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="button">{{ trans('cruds.home.fields.button') }}</label>
                <input class="form-control {{ $errors->has('button') ? 'is-invalid' : '' }}" type="text" name="button" id="button" value="{{ old('button', $home->button) }}">
                @if($errors->has('button'))
                    <span class="text-danger">{{ $errors->first('button') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.home.fields.button_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="image">{{ trans('cruds.home.fields.image') }}</label>
