...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.brands.update", [$brand->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.brand.fields.language') }}</label>
                <select class="form-control {{ $errors->has('language') ? 'is-invalid' : '' }}" name="language" id="language" required>
                    ...
                    @foreach(App\Models\Brand::LANGUAGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('language', $brand->language) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('language'))
                    <span class="text-danger">{{ $errors->first('language') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.brand.fields.language_helper') }}</span>
            </div>
            <div class="form-group">
