...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.industries.update", [$industry->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.industry.fields.language') }}</label>
                <select class="form-control {{ $errors->has('language') ? 'is-invalid' : '' }}" name="language" id="language" required>
                    ...
                    @foreach(App\Models\Industry::LANGUAGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('language', $industry->language) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('language'))
                    <span class="text-danger">{{ $errors->first('language') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.industry.fields.language_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $industry->slug) }}" required>
                @if($errors->has('slug'))
