        ...
        <form method="POST" action="{{ route("admin.translation-centers.update", [$translationCenter->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('online') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="online" id="online" value="1" {{ $translationCenter->online || old('online', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('language') ? 'is-invalid' : '' }}" name="language" id="language" required>
                    ...
                    @foreach(App\Models\TranslationCenter::LANGUAGE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('language', $translationCenter->language) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $translationCenter->slug) }}" required>
                @if($errors->has('slug'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('section') ? 'is-invalid' : '' }}" name="section" id="section" required>
                    ...
                    @foreach(App\Models\TranslationCenter::SECTION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('section', $translationCenter->section) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
