...
#     UNION
    ...
    SELECT id+200, IF(online > 0, 1, 0) AS online, dt as created_at, id AS oldid, img AS oldimage, 'News' AS oldarticletype
...
SELECT * FROM (
    ...
    SELECT IF(online > 0, 1, 0) AS online, 'ro'  AS locale, trim(nume) AS name, trim(slug), content, img1 as oldimg1, img2 as oldimg2, img3 as oldimg3, 'Noutati' as oldcategory, id+200 as blog_id
    FROM minex_live.noutati WHERE del='0' and nume!=''
        UNION
    SELECT IF(online > 0, 1, 0) AS online, 'en'  AS locale, trim(nume1) AS name, trim(slug1) AS slug, content1 as content, img11 as oldimg1, img21 as oldimg2, img31 as oldimg3, 'News' as oldcategory, id+200 as blog_id
    FROM minex_live.noutati WHERE del='0' and nume1!=''
        UNION
    SELECT IF(online > 0, 1, 0) AS online, 'bg' AS locale, trim(nume2) AS name, trim(slug2) AS slug, content2 as content, img12 as oldimg1, img22 as oldimg2, img32 as oldimg3, 'Новини' as oldcategory, id+200 as blog_id
